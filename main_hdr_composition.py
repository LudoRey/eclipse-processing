...
from parameters import MERGED_STACKS_DIR

from disk import binary_disk
from hdr import saturation_weighting, masked_resampled_linear_trigo_fit_images, angle_map
...
filepaths_per_exptime = get_filepaths_per_exptime(MERGED_STACKS_DIR)
...
moon_mask = binary_disk(header["MOON-X"], header["MOON-Y"], radius=moon_radius_pixels, shape=shape[0:2])
...
weights = saturation_weighting(img_y.max(axis=2), 0, HIGH_CLIPPING_THRESHOLD, LOW_SMOOTHNESS, HIGH_SMOOTHNESS) * header_y["EXPTIME"]**2
...
for i in reversed(range(len(exptimes)-1)):
    ...
    if i == 0:
        weights = saturation_weighting(img_x.max(axis=2), LOW_CLIPPING_THRESHOLD, 1, LOW_SMOOTHNESS, HIGH_SMOOTHNESS) * header_y["EXPTIME"]**2
    else:
        weights = saturation_weighting(img_x.max(axis=2), LOW_CLIPPING_THRESHOLD, HIGH_CLIPPING_THRESHOLD, LOW_SMOOTHNESS, HIGH_SMOOTHNESS) * header_y["EXPTIME"]**2
    # Compute scaled irradiance
    ...
    # Fit image
    print("Fitting image...")
    img_fitted_x = masked_resampled_linear_trigo_fit_images(img_x, img_theta, img_y, mask)

    # Add weighted scaled irradiance to the HDR image
    hdr_img += weights[:,:,None] * img_fitted_x
    sum_weights += weights
...
save_as_fits(hdr_img, output_header, os.path.join(HDR_DIR, "hdr.fits"), convert_to_uint16=False)

# vmax = img_fitted_x.max() 
# img_y = np.clip(img_y/vmax, 0, 1)
# img_x = np.clip(img_x/vmax, 0, 1)
# img_fitted_x = np.clip(img_fitted_x/vmax, 0, 1)

# save_as_float_fits(img_y, header_y, os.path.join(HDR_DIR, f"{float(exptimes[-1]):.5f}s.fits"))
# save_as_float_fits(img_fitted_x, header_x, os.path.join(HDR_DIR, f"{float(exptimes[i]):.5f}s.fits"))
# save_as_float_fits(img_x, header_x, os.path.join(HDR_DIR, f"old_{float(exptimes[i]):.5f}s.fits"))

#fig1, axes = plt.subplots(1,2)
#axes[0].imshow(ht(crop(img_x, int(header_x["SUN-X"]), int(header_x["SUN-Y"]), 1024, 1024), m=0.1))
#axes[1].imshow(ht(crop(img_x, int(header_x["SUN-X"]), int(header_x["SUN-Y"]), 1024, 1024), m=0.1))
